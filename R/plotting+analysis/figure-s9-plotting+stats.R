# Name: figure-S9-plotting+stats.R
...
ggsave(filename = here::here("plot_outputs/FigS9_sens_scatter_varscale.pdf"), plot = sensplot,
...
ggsave(filename = here::here("plot_outputs/FigS9_threshold_frequency.pdf"), 
...
summary_df %>% filter(Actual == 1) %>% nrow()
...
ggsave(filename = here::here("plot_outputs/FigS9_hotspot_frequency.pdf"), 
       plot = last_plot(), device = 'pdf', width = 190/1.5, height = 100/1.5, 
       units = 'mm', dpi = 400)
